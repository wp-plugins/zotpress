        ...
        */
        ...
        function FilterCitations(account, collection_id, limit)
        {
            ...
            var xmlUriCitationsByFilter = '<?php echo ZOTPRESS_PLUGIN_URL; ?>zotpress.rss.php?'
                                                    + 'account_type='+account.account_type
                                                    + '&api_user_id='+account.api_user_id
                                                    + '&public_key='+account.public_key
            ...
            var xmlUriCitationFilterImages = '<?php echo ZOTPRESS_PLUGIN_URL; ?>zotpress.rss.php?'
                                        + 'account_type='+account.account_type
                                        +'&api_user_id='+account.api_user_id
                                        +'&public_key='+account.public_key
                                        +'&displayImages=true';
            ...
            // Grab Images
            jQuery.get(xmlUriCitationFilterImages, {}, function(xml)
            {
                jQuery(xml).find("zpimage").each(function()
                {
                    var zpimage = jQuery(this);
            ...
            if (account.id != current_account)
            {
                var xmlUriFilterCollections = '<?php echo ZOTPRESS_PLUGIN_URL; ?>zotpress.rss.php?'
                                                + 'account_type='+account.account_type
                                                +'&api_user_id='+account.api_user_id
                                                +'&public_key='+account.public_key
                                                +'&data_type=collections';
                ...
                // Grab Collections
                jQuery.get(xmlUriFilterCollections, {}, function(xml)
                {
                    if (browser_is_IE)
                    {
                        xml = createXmlDOMObject (xml);
